...
class ShadowKVCache:
    ...
    def __init__(self, 
        ...
        # Recall measurement is opt-in because retaining exact prefill keys costs
        # about 2 GiB for Llama-3.1-8B at 32K context.
        self.recall_enabled = False
        self.recall_topk = self.sparse_budget
        self.recall_key_cache = None
        self.recall_outlier_position_ids = [None] * self.num_layers
        self.last_recall_stats = None
        self._reset_recall_stats()

    def enable_recall_stats(self, topk):
        self.recall_enabled = True
        self.recall_topk = int(topk)
        self.recall_key_cache = torch.empty(
            self.num_layers,
            self.batch_size,
    ...
    def _reset_recall_stats(self):
        self.recall_hits = torch.zeros((), dtype=torch.int64, device=self.device)
        self.recall_selected = torch.zeros((), dtype=torch.int64, device=self.device)
        self.recall_total = 0
        self.recall_events = 0
        self.last_recall_stats = None

    def get_recall_stats(self, use_snapshot=False):
        if use_snapshot and self.last_recall_stats is not None:
            return dict(self.last_recall_stats)
        hits = int(self.recall_hits.item())
        selected = int(self.recall_selected.item())
        return {
            "recall_hits": hits,
            "recall_total": self.recall_total,
            "selection_recall": hits / self.recall_total if self.recall_total else None,
            "selected_tokens_total": selected,
            "recall_events": self.recall_events,
            "mean_selected_tokens": (
                selected / self.recall_events if self.recall_events else None
            ),
        }

    def snapshot_recall_stats(self):
        if self.recall_enabled:
            self.last_recall_stats = self.get_recall_stats()
    ...
    def prefill_kv_cache(self,
        ...
        if self.recall_enabled:
            self.recall_key_cache[layer_idx, :, :, :incoming].copy_(key_states_roped)
        ...
        # ``-0:`` means the whole tensor in Python, not an empty suffix.
        if self.prefill_local > 0:
        ...
        if self.recall_enabled:
            offsets = torch.arange(self.chunk_size, device=self.device)
            self.recall_outlier_position_ids[layer_idx] = (
                outlier_chunk_idx.unsqueeze(-1) * self.chunk_size + offsets
    ...
    def get_retrieval_position_ids(self, layer_idx, query_states):
        ...
        if self.recall_enabled:
            self._update_recall(layer_idx, query_states, position_ids)

        return position_ids

    def _update_recall(self, layer_idx, query_states, position_ids):
        """Compare ShadowKV's shared GQA candidate set with exact Top-K."""
        prompt_len = self.prefill
        exact_keys = self.recall_key_cache[layer_idx, :, :, :prompt_len]
        ...
        exact_scores = torch.einsum("bhgqd,bhkd->bhgqk", queries, exact_keys)
        k = min(self.recall_topk, prompt_len)
        truth = exact_scores.topk(k, dim=-1).indices

        candidates = [position_ids]
        outlier_ids = self.recall_outlier_position_ids[layer_idx]
        if outlier_ids is not None and outlier_ids.shape[-1] > 0:
            candidates.append(outlier_ids)
        if self.prefill_local > 0:
            local_ids = torch.arange(
                prompt_len - self.prefill_local,
                prompt_len,
                device=self.device,
            ).view(1, 1, -1).expand(
        ...
        expanded_mask = candidate_mask[:, :, None, None, :].expand_as(exact_scores)
        self.recall_hits.add_(
            expanded_mask.gather(-1, truth).sum(dtype=torch.int64)
        )
        events = truth.numel() // k
        self.recall_selected.add_(
            candidate_mask.sum(dtype=torch.int64)
            * self.num_key_value_groups
            * self.incoming_q_len
        )
        self.recall_total += truth.numel()
        self.recall_events += events
...
class ShadowKVCache_CPU:
    ...
    def __init__(self, 
        ...
        self.rank = rank
        self.local_chunk = 4
        # 保留上游分配规则：512时0，1024时24；landmark数量需按8对齐。
        self.outlier_chunk = (self.sparse_budget // 1024) * 24
        ...
        self.k_cache_buffer = torch.zeros(
            ...
            batch_size,
            config.num_key_value_heads,
            self.sparse_budget + 128 + (self.outlier_chunk+self.local_chunk+7)*self.chunk_size,
            self.config.hidden_size // self.config.num_attention_heads,
            device=self.device,
            dtype=self.dtype
        )
