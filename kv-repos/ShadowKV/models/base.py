...
class LLM:
    ...
    def layer_compute(self, 
        ...
        if isinstance(self.kv_cache, KV_Cache):
            ...
            if q_len > 1: # prefill
                ...
                # flash attention
                if getattr(self.kv_cache, "strict_total_budget", False):
                    value_states.record_stream(curr_stream)
                    hidden_states = self.kv_cache.attention(query_states, key_states, value_states, layer_idx)
                else:
                    hidden_states = flash_attn_with_kvcache(q=query_states.transpose(1, 2), k_cache=key_states.transpose(1, 2), v_cache=value_states.transpose(1, 2), causal=True)
    ...
    def generate(self, input_ids: torch.Tensor, gen_len: int = 256, temperature: float = 0.0, top_p: float = 0.9, top_k :int = 50, verbose: bool = False, benchmark: bool = False, cont: bool = False):
        ...
        # The final feed only updates cache state for continued generation.  Its
        # attention does not contribute to generated_ids, so exclude it from
        # the reported selector recall.
        if hasattr(self.kv_cache, "snapshot_recall_stats"):
            self.kv_cache.snapshot_recall_stats()
