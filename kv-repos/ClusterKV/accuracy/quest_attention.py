...
def quest_attn_out(query_states, key_states, value_states, attention_mask, 
                   ...
                   cluster_cache, topk_stat=False, recall_stats=None,
                   num_key_value_groups=1, gqa_policy=None):
    ...
    if gqa_policy == "qavg":
        # Select once per KV head. The resulting token/page set is then shared
        # by all query heads that use that KV head.
        attn_weights_for_selection = average_query_heads(
            attn_weights_for_selection,
    ...
    if token_budget > 0:
        ...
        if gqa_policy == "qavg":
            mask_bottom = repeat_kv_head_selection(
                mask_bottom,
                num_key_value_groups,
            )
            mask_bottom_full = repeat_kv_head_selection(
                mask_bottom_full,
                num_key_value_groups,
            )
        update_from_mask(
            recall_stats,
            mask_bottom,
            attn_weights[..., :mask_bottom.shape[-1]],
            token_budget,
        )
    else:
...
def forward_quest(
    ...
    **kwargs,
    ...
    if strict and (self.gqa_policy != "qavg" or self.gen):
        raise ValueError("strict Quest requires qavg and selection over all history")
    ...
    attn_output = quest_attn_out(query_states, key_states, value_states, attention_mask,
                                 ...
                                 self.cluster_cache, self.topk_stat, self.recall_stats,
