...
from .online_cluster import append_decode_keys
from .recall_stats import update_from_indices
...
def cluster_attn_out(query_states, key_states, value_states, attention_mask, prompt_len,
                    ...
                    recall_stats=None, gqa_policy=None):
    bsz, num_kv_heads, kv_seq_len, head_dim = key_states.shape
    _, num_heads, q_len, _ = query_states.shape
    hidden_size = num_heads * head_dim
	# c_dist: (1, selection_heads, 1, nlist)
    selection_queries = query_states
    if gqa_policy == "qavg":
        selection_queries = average_query_heads(
            query_states,
            num_key_value_groups,
        )
    c_dist = torch.matmul(selection_queries, key_centroids.transpose(2, 3))
    ...
    if use_search_kernel:
        ...
        num_selection_heads = c_neighbor.shape[0]
        sel_key_indices = torch.full((num_selection_heads, max_num_indices), kv_seq_len,
    ...
    if gqa_policy == "qavg":
        sel_key_indices = repeat_kv_head_selection(
            sel_key_indices,
            num_key_value_groups,
        )
    ...
    update_from_indices(
        recall_stats,
        full_sel_key_indices if topk_stat else torch.cat([
            torch.arange(sink, device=sel_key_indices.device).repeat(1, num_heads, 1),
            sel_key_indices,
        ], dim=-1),
        torch.matmul(
            query_states,
            key_states[:, :, :prompt_len, :].transpose(2, 3),
        ),
        token_budget,
    )
...
def forward_cluster(
    ...
    **kwargs,
    ...
    if strict and (self.gqa_policy != "qavg" or self.balance or self.head_sel != "truc"):
        raise ValueError("strict ClusterKV requires qavg, unbalanced clusters and truc")
    past_len = past_key_value.get_seq_length(self.layer_id) if past_key_value is not None else 0
    use_dense = past_len + q_len <= self.token_budget if strict else (
        (self.prompt_len == 0 and q_len < self.token_budget)
        or (self.prompt_len > 0 and self.prompt_len + q_len < self.token_budget)
    )

    if q_len > 1 or use_dense:
        if q_len > 1:
    ...
    sink = self.sink
    prefill_key = layer_kv(past_key_value, self.layer_id)[0]
    assert prefill_key.shape[-2] > sink
    prefill_key = prefill_key[..., sink:, :]
    # clustering for prefilled keys
    if self.key_centroids is None:
        nlist = min(self.nlist, prefill_key.shape[-2]) if strict else self.nlist
        params = self.cluster_params
        if strict and nlist != self.nlist:
            params = KMeansParams(n_clusters=nlist, max_iter=params.max_iter, metric="cosine")
        self.key_centroids, self.cluster_key_indices, \
        self.cluster_key_ptr, self.cluster_key_size, self.cluster_key_size_ps = \
		build_cluster(prefill_key, nlist, self.balance, params,
    ...
    if past_key_value is not None:
        kv_seq_len += layer_kv(past_key_value, self.layer_id)[0].shape[-2]
    ...
    selection_length = self.prompt_len
    if strict:
        append_decode_keys(self, key_states, sink)
        selection_length = key_states.shape[-2]

    key_states = repeat_kv(key_states, self.num_key_value_groups)
    value_states = repeat_kv(value_states, self.num_key_value_groups)

    token_budget = min(selection_length, self.token_budget)
    attn_output = cluster_attn_out(
        query_states, key_states, value_states, attention_mask, 
        selection_length, self.key_centroids, self.cluster_key_indices,
        ...
        self.recall_stats, self.gqa_policy
    )
...
def cluster_reset(model):
    # Release every tensor backed by the previous request before replacing the
    # RMM resource.  The old code rebuilt a hard-capped 1 GiB pool first, while
    # those tensors were still live, and failed after a few requests.
    torch.cuda.synchronize()
    for module in model.modules():
        if hasattr(module, "cluster_params"):
            for name in ("key_centroids", "cluster_key_indices", "cluster_key_ptr",
                         "cluster_key_size", "cluster_key_size_ps"):
                setattr(module, name, None)
    gc.collect()
    torch.cuda.empty_cache()
    # Start at 1 GiB but allow RMM to grow when RAFT KMeans needs more workspace.
    rmm.reinitialize(pool_allocator=True, initial_pool_size=1 * 1024**3)
