...
class QuestAttention(nn.Module):
    ...
    def _init_rope(self):
        ...
        if rs is None:
            ...
            # NOTE(patch): transformers>=4.44 names this key "rope_type", older configs use "type".
            scaling_type = rs.get("type", rs.get("rope_type"))
            if scaling_type == "linear":
                # support for Longchat-v1.5.
                self.rope_scale = rs["factor"]
            elif scaling_type in ("llama3", "dynamic", "yarn", "longrope"):
                # Segmented-frequency schemes cannot be expressed by the kernel's single
                # scalar scale. Fall back to no extra scaling; correctness then relies on
                # the true rope_theta being passed at the call site.
                self.rope_scale = 1.0
            else:
    ...
    def forward(
        ...
        clusterkv.utils.apply_rope_in_place(
            ...
            rope_theta=getattr(self.config, "rope_theta", None) or 1e4, 
        )
        torch.cuda.nvtx.range_pop()
