...
def decode_sparse_attn(
    ...
    # ---- offload path: KV master copy lives in CPU DRAM;
    #      only the top-k selected pages are staged into GPU this step.
    if iController.offload_layer(layer_idx):
        st = iController.kv_offload
        st.sync_from_gpu(iController)                  # incremental D2H (only on page fill)
        slot, n_uniq = st.recall(layer_idx, topk_indices, iController)   # <-- LOAD (H2D)
        iController._decode_handler.forward(
            q,
            o,
            st.staging_view(layer_idx, n_uniq),        # compact staging replaces full kv_cache
            slot,                                       # per-head slot mapping
            iController.kv_indptr_for_approx_decode,
            iController.kv_cache.last_page_len,
            n_uniq,                                     # slot of the current (in-flight) page
            rope_scale,
            rope_theta,
        )
        return o
