...
class InferenceController:
    def __init__(
        ...
        # ---------------------------------------------------------- offload
        # KV 主副本放 CPU(DRAM)，GPU 只留 metadata(常驻) + staging(本步选中页)
        self.offload = offload
        self.kv_offload = None
        if offload:
            ext = None
            try:
                from clusterkv.utils.quest_offload_ext import load_ext
                ext = load_ext()
                print("[offload] GPU gather kernel (UVA zero-copy) ready")
            except Exception as e:
                print("[offload] GPU gather ext unavailable, "
                      f"falling back to CPU gather: {e}")
            self.kv_offload = OffloadKvStore(
                num_layers=num_layers,
                num_kv_heads=num_kv_heads,
                head_dim=head_dim,
                max_seq_len=max_seq_len,
                ...
                dtype=dtype,
                device=device,
                max_sel_pages=num_heads * page_budget,   # 每 head 最多 page_budget 页
                ext=ext,
            )
    ...
    def clean_states(self):
        ...
        if self.kv_offload is not None:
            self.kv_offload._synced_pages = 0

    # ------------------------------------------------------------------
    # offload 的层选择
    # 前 SKIP_LAYERS 层不做稀疏（need_estimate()==False，会扫全部页），
    # 因此也不 offload —— 它们的 KV 常驻 GPU。这与 ClusterKV offload
    # 的 layer_idx >= 2 策略保持一致，便于横向对比。
    SKIP_LAYERS = 2

    def offload_layer(self, layer_idx: int) -> bool:
        return self.offload and layer_idx >= self.SKIP_LAYERS
