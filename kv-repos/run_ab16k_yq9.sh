...
for r in a b; do
  echo "##### round $r full #####"
  python scripts/bench_decode.py --method full      $C --tag ab16k_${r}_full      --out_json results/ab16k_${r}_full.json; echo "[exit=$?]"
  echo "##### round $r clusterkv #####"
  python scripts/bench_decode.py --method clusterkv $C --tag ab16k_${r}_clusterkv --out_json results/ab16k_${r}_clusterkv.json; echo "[exit=$?]"
done
echo "##### DONE #####"
