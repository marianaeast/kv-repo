...
LIMIT=${1:-0}
RUN_NAME=${RUN_NAME:-hotpotqa_k128_recall_$(date +%Y%m%d_%H%M%S)}
...
COMMON=(--model llama3.1-8b-chat-32k --task hotpotqa --token_budget 128
        ...
        --output_root "$OUT/cluster_driver" --resume --recall_stat)
...
PYTHONPATH="$SHADOW_ROOT" "$SHADOW_PY" -u test/eval_hotpotqa_equal_budget.py \
    ...
    --data_path "$DATA" --output "$OUT/shadowkv_recall.jsonl" \
    --total_budget 128 --chunk_size 8 --rank 160 --limit "$LIMIT" \
    --device cuda:0 --resume --recall_stat

MODEL_DIR=$OUT/cluster_driver/llama3.1-8b-chat-32k
/home/zrd/miniconda3/envs/vllm-cu129/bin/python "$ROOT/summarize_hotpotqa_recall.py" \
    --quest "$MODEL_DIR/hotpotqa-128-recall.jsonl" \
    --clusterkv "$MODEL_DIR/hotpotqa-htruc400fi20sink16_128-recall.jsonl" \
    --shadowkv "$OUT/shadowkv_recall.jsonl" --output "$OUT/summary.csv"
